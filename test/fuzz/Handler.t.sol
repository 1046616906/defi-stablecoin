...
contract Handler is Test {
    ...
    function redeemCollateral(uint256 collateralSeed, uint256 collateralAmount) public {
        ...
        if (totalDscMinted == 0) {
            ...
            maxCollateralToRedeemInUsd = collateralValueInUsd - minCollateralRequired;
        }

        // 4. 将 USD 价值转换回 Token 数量
        uint256 maxTokenToRedeem = dscEngine.getTokenAmountFormUsd(collateralToken, maxCollateralToRedeemInUsd);

        // 5. 还要和账面余额取最小值（防止精度误差）
        uint256 userBalance = dscEngine.getUserCollateralAmount(sender, collateralToken);
    ...
    function mintDsc(uint256 amount, uint256 addressSeed) public {
        ...
        (uint256 totalDscMinted, uint256 collateralValInUsd) = dscEngine.getAccountInformation(sender);

        int256 maxMintDscAmount = int256(collateralValInUsd) / 2 - int256(totalDscMinted);
    ...
    function _getCollateralSeed(uint256 collaterSeed) internal view returns (address) {
        if (collaterSeed % 2 == 0) {
            return weth;
        }
        return wbtc;
