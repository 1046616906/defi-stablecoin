...
contract DSCEngineTest is Test {
    ...
    // burnDsc test ============================================
    modifier depositedAndMint() {
        vm.startPrank(USER);
        ERC20Mock(weth).approve(address(dscEngine), INIT_AMOUNT);
        dscEngine.depositCollateralAndMintDsc(weth, INIT_AMOUNT, 100 ether);
        vm.stopPrank();
        _;
    }

    function test_burnDsc() external depositedAndMint {
        uint256 burnDscAmount = 50 ether;
        vm.startPrank(USER);
